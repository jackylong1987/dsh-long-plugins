window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      /* 输入区随系统深/浅切换：文字与背景都用系统色(Canvas/CanvasText)，避免深色下白字配浅底 */
      .uV2eYG_input{background-color:Canvas!important;color:CanvasText!important;color-scheme:light dark}
    ...
    const GLASS_CSS = `
      ...
      /* 输入框盒子(uV2eYG_card)：只给输入框铺一层可调的淡色底(输入区颜色/罩强度/透明度控制)，不打磨砂 */
    ...
    const GlassSettingsSection = () => {
      ...
      return React.createElement('div', { className: 'dsh-glass-section' },
        ...
        React.createElement('div', { className: 'dsh-glass-card' },
          ...
          React.createElement('p', { className: 'dsh-glass-note' }, '提示：背景图铺满整页(可加背景图罩)；背景图模糊/透明度单独可调；会话区/输入区各自使用自己的罩色、罩强度与透明度；左栏/顶部透出背景图。'),
