  ...
  const section = (group) => {
    ...
    const rows = group.files.map((f) => {
      ...
      return `<tr>
        ...
        <td class="acts">
          <a class="tag" href="workspace-preview?path=${rel}">${ICON_EYE} 预览</a>
...
<style>
  * { box-sizing: border-box; }
  ...
  .seg a { display:inline-flex; align-items:center; gap:6px; padding:8px 18px; font-size:14px; text-decoration:none; color:#9ca3af; background:transparent; }
  ...
  .bar .spacer { flex:1; }
  .btn { display:inline-flex; align-items:center; gap:6px; background:#374151; color:#e5e7eb; text-decoration:none; border-radius:8px; padding:7px 14px; font-size:13px; flex:none; }
  ...
  .tag { display:inline-flex; align-items:center; gap:4px; text-decoration:none; font-size:12px; padding:4px 10px; border-radius:6px; margin-right:6px; background:#1e293b; color:#93c5fd; }
...
<style>
  * { box-sizing: border-box; }
  ...
  .btn2 { background:transparent; color:#e5e7eb; border:1px solid #2c3a47; cursor:pointer; }
  .btn2:hover { background:#2c3a47; }
...
function closePreview() {
  try {
    if (window.self !== window.top && window.parent) {
      // 在 iframe（会话内联预览）里：
      // 若 iframe 内部有历史（从浏览列表点进来的）→ 返回列表，只关掉打开的文件
      if (window.history.length > 1) { window.history.back(); return; }
      // 直接打开的单文件预览 → 通知父窗口关闭整个面板
