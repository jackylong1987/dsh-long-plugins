...
function workspaceBrowseHtml(groups, ws = "", all = false) {
  ...
  const section = (group) => {
    ...
    const rows = group.files.map((f) => {
      ...
      return `<tr>
        <td class="name"><a class="flink" href="workspace-preview?path=${rel}&from=list" title="${escapeHtml(f.path)}">${escapeHtml(f.name)}</a></td>
        <td class="size">${humanSize(f.size)}</td>
        <td class="time">${humanTime(f.mtime)}</td>
        <td class="acts">
          <a class="tag" href="workspace-preview?path=${rel}&from=list">${ICON_EYE} 预览</a>
...
<style>
  * { box-sizing: border-box; }
  ...
  .pdf { width:100%; height:85vh; border:1px solid var(--lp-border); border-radius:8px; background:#fff; }
