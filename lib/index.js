...
export function createHandlers(options = {}) {
  ...
  const workspacePreview = async (req, res) => {
    try {
      ...
      if (OFFICE_EXTS.has(ext)) {
        ...
        body = `<iframe class="pdf" src="data:application/pdf;base64,${buffer.toString("base64")}"></iframe>`;
      } else if (/\.(txt|md|markdown|json|ya?ml|py|js|mjs|cjs|ts|sh|css|html?|xml|csv|log|ini|conf|env|toml|sql|rs|go|c|h|cpp|java|kt|swift|rb|php|vue|jsx|tsx)$/i.test(name)) {
        body = `<pre class="text">${escapeHtml(buffer.toString("utf8"))}</pre>`;
      } else {
        body = `<p class="unsupported">该文件类型暂不支持预览，请点击右上角「下载」。</p>`;
      }
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" });
      res.end(previewPageHtml(name, rel, size, downloadHref, body));
    } catch (error) {
      sendError(res, error, onError);
    }
  };
  ...
  return { root, maxFileBytes, totalMaxBytes, api, download, preview, workspaceList, workspaceFile, workspacePreview, workspaceBrowse, workspaceDelete, workspaceSave };
}
...
function workspaceBrowseHtml(groups, ws = "", all = false) {
  ...
  const section = (group) => {
    ...
    const rows = group.files.map((f) => {
      ...
      return `<tr>
        <td class="name"><a class="flink" href="workspace-preview?path=${rel}" title="${escapeHtml(f.path)}">${escapeHtml(f.name)}</a></td>
        <td class="size">${humanSize(f.size)}</td>
        <td class="time">${humanTime(f.mtime)}</td>
        <td class="acts">
          <a class="tag" href="workspace-preview?path=${rel}">👁 预览</a>
          <a class="tag dl" href="workspace-file?path=${rel}&download=1">⬇ 下载</a>
        </td>
      </tr>`;
    }).join("");
...
<style>
  * { box-sizing: border-box; }
  body { margin:0; font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif; background:#0f1720; color:#e5e7eb; }
  .bar { position:sticky; top:0; display:flex; align-items:center; gap:14px; padding:12px 20px; background:#1a2530; border-bottom:1px solid #2c3a47; z-index:5; flex-wrap:wrap; }
  .seg { display:flex; border:1px solid #2c3a47; border-radius:10px; overflow:hidden; flex:none; }
  .seg a { padding:8px 18px; font-size:14px; text-decoration:none; color:#9ca3af; background:transparent; }
  .seg a + a { border-left:1px solid #2c3a47; }
  .seg a.on { background:#2563eb; color:#fff; }
  .seg a:hover:not(.on) { background:#273449; color:#e5e7eb; }
  .bar .meta { color:#9ca3af; font-size:13px; }
  .bar .spacer { flex:1; }
  .btn { background:#374151; color:#e5e7eb; text-decoration:none; border-radius:8px; padding:7px 14px; font-size:13px; flex:none; }
  .btn:hover { background:#4b5563; }
  .wrap { max-width:980px; margin:0 auto; padding:20px; }
  .group { margin-bottom:26px; }
  .group h2 { font-size:15px; margin:0 0 8px; color:#93c5fd; }
  .cnt { color:#6b7280; font-size:12px; font-weight:400; }
  table { width:100%; border-collapse:collapse; background:#111a24; border:1px solid #263241; border-radius:10px; overflow:hidden; }
  th, td { text-align:left; padding:9px 14px; font-size:13px; border-bottom:1px solid #1c2836; }
  th { background:#16222f; color:#9ca3af; font-weight:500; }
  tr:last-child td { border-bottom:none; }
  tr:hover td { background:#17232f; }
  .name { max-width:380px; }
  .flink { color:#e5e7eb; text-decoration:none; word-break:break-all; }
  .flink:hover { color:#93c5fd; text-decoration:underline; }
  .size, .time { color:#9ca3af; white-space:nowrap; }
  .acts { white-space:nowrap; }
  .tag { display:inline-block; text-decoration:none; font-size:12px; padding:4px 10px; border-radius:6px; margin-right:6px; background:#1e293b; color:#93c5fd; }
  .tag:hover { background:#273449; }
  .tag.dl { color:#86efac; }
  ...
  @media (max-width: 640px) {
    ...
    thead { display:none; }
    tbody { display:block; }
    tr { background:#111a24; border:1px solid #263241; border-radius:10px; padding:10px 12px; margin-bottom:8px; }
...
<div class="bar">
  <div class="seg">
    <a class="${!all && ws ? "on" : ""}" href="${wsHref || "#"}" ${wsHref ? "" : 'aria-disabled="true" title="从会话窗口的「📂 文件」进入可回到当前工作区"'}>📁 工作区文件</a>
    <a class="${all ? "on" : ""}" href="${allHref}">📂 总文件</a>
  </div>
...
function previewPageHtml(name, rel, size, downloadHref, body) {
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
...
<style>
  * { box-sizing: border-box; }
  body { margin:0; font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif; background:#0f1720; color:#e5e7eb; }
  .bar { position:sticky; top:0; display:flex; align-items:center; gap:12px; padding:10px 16px; background:#1a2530; border-bottom:1px solid #2c3a47; }
  .bar .name { font-weight:600; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .bar .meta { color:#9ca3af; font-size:12px; }
  .bar .spacer { flex:1; }
  .btn { background:#2563eb; color:#fff; text-decoration:none; border-radius:8px; padding:7px 16px; font-size:14px; }
  .btn:hover { background:#1d4ed8; }
  .btn2 { background:#374151; color:#e5e7eb; border:none; border-radius:8px; padding:7px 16px; font-size:14px; cursor:pointer; }
  ...
  .text { background:#0b1219; border:1px solid #2c3a47; border-radius:10px; padding:16px; overflow:auto; font-size:13px; line-height:1.7; white-space:pre-wrap; word-break:break-word; }
  .image { max-width:100%; border-radius:8px; }
  .pdf { width:100%; height:85vh; border:1px solid #2c3a47; border-radius:8px; background:#fff; }
...
<div class="bar">
  ...
  <span class="spacer"></span>
  <a class="btn" href="${escapeHtml(downloadHref)}" download>⬇ 下载</a>
  <button class="btn2" type="button" onclick="closePreview()">✕ 关闭</button>
</div>
