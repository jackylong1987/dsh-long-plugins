...
<style>
  * { box-sizing: border-box; }
  ...
  .fsearch-box { position:absolute; right:0; top:calc(100% + 6px); z-index:30; display:flex; align-items:center; gap:6px; background:var(--lp-bar-bg); border:1px solid var(--lp-border); border-radius:10px; padding:6px; box-shadow:var(--lp-shadow-lv3, 0 10px 30px rgba(0,0,0,.35)); }
  ...
  @media (max-width: 767px) { .fsearch-box { position:fixed; right:auto; left:16px; top:76px; max-width:calc(100vw - 32px); width:max-content; } }
...
(function () {
  ...
  function setFsearchOpen(open) {
    fsearchOpen = open;
    if (fsearchBox) fsearchBox.style.display = open ? 'flex' : 'none';
  ...
  if (fsearchClear) fsearchClear.addEventListener('click', function (e) { e.preventDefault(); e.stopPropagation(); fsearch.value = ''; apply(); setFsearchOpen(false); });
  if (fsearchWrap) document.addEventListener('mousedown', function (e) { if (fsearchOpen && !fsearchWrap.contains(e.target)) setFsearchOpen(false); });
