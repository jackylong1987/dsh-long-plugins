...
<div style="display:flex;gap:8px;padding:10px 14px;align-items:center;background:#1a2530">
...
(async()=>{
  ...
  const docxPreviewAsset = async (req, res) => {
    try {
      ...
      if (f !== "jszip.min.js" && f !== "docx-preview.min.js") throw new HttpError(400, "unknown asset");
  ...
  return { root, maxFileBytes, totalMaxBytes, api, download, preview, workspaceList, workspaceFile, workspacePreview, workspaceBrowse, workspaceDelete, workspaceSave, docxPreviewPage, docxPreviewAsset };
}
  ...
  const section = (group) => {
    ...
    const rows = group.files.map((f) => {
      ...
      const viewHref = previewable
        ? (isPdf ? `workspace-file?path=${rel}&inline=1` : isDocx ? `docx-preview?path=${rel}` : `workspace-preview?path=${rel}&from=list`)
