...
<style>
  * { box-sizing: border-box; }
  ...
  .dayfilter { background:var(--lp-btn-bg); color:var(--lp-btn-fg); border:1px solid var(--lp-border); border-radius:8px; padding:5px 8px; font-size:13px; color-scheme:dark; }
  .dayfilter::-webkit-calendar-picker-indicator { cursor:pointer; filter:invert(.6); }
  ...
  @media (prefers-color-scheme: light) { .dayfilter { color-scheme:light; } .dayfilter::-webkit-calendar-picker-indicator { filter:none; } }
...
<div class="bar">
  ...
  <span class="daybox">
    ...
    <input type="date" id="dayFilter" class="dayfilter">
    <button type="button" class="btn2 daybtn" id="dayToday">今天</button>
    <button type="button" class="btn2 daybtn" id="dayClear">全部</button>
  </span>
...
(function () {
  ...
  var dayClear = document.getElementById('dayClear');
  ...
  function apply() {
    ...
    if (dayClear) dayClear.classList.toggle('active', !sel);
  ...
  if (dayClear) dayClear.addEventListener('click', function () { dayInput.value = ''; apply(); });
  apply();
})();
</script>
</body>
