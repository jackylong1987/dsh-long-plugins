...
/** Resolve a user-supplied relative path strictly inside the root. */
function safeResolveInside(root, rel) {
...
/** Read-only resolver: relative paths stay inside root; absolute host paths are
 * allowed as-is so deliverable files presented outside the workspace can preview.
 * Destructive endpoints MUST use safeResolveInside. */
function safeResolve(root, rel) {
  if (typeof rel === "string" && rel.length > 0 && !rel.includes("\0") && isAbsolute(rel)) {
    return normalize(rel);
  }
  return safeResolveInside(root, rel);
}
...
export function createHandlers(options = {}) {
  ...
  const workspaceDelete = async (req, res) => {
    try {
      ...
      const full = safeResolveInside(workspaceRoot, rel);
      if (full === undefined) throw new HttpError(400, "invalid path");
      const info = await stat(full);
      if (!info.isFile()) throw new HttpError(400, "not a regular file");
      await unlink(full);
  ...
  const workspaceRename = async (req, res) => {
    try {
      ...
      const newFull = safeResolveInside(workspaceRoot, newRel);
...
<style>
 ...
 /* 页面尺寸与页边距**交给 docx-preview 按文档 sectPr 计算**：这里不再覆盖 padding/max-width/min-height，
    否则页边距与纸型会与 Word 不一致（这是"预览不像"的主要原因）。只加白纸、阴影、页间距。 */
 #container .docx-wrapper > .docx,
 #container .docx-wrapper > section.docx,
 #container .docx-wrapper section.docx{background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.28);margin:0 0 24px;box-sizing:border-box}
 #container .docx-wrapper section.docx + section.docx{margin-top:0}
 ...
 /* 「显示修订」开启时：docx-preview 用 HTML <ins>/<del> 元素渲染插入/删除（带 dateTime），
    浏览器默认样式偏淡，这里给足对比度，与 Word 的修订观感一致。 */
 #container ins{color:#047857;text-decoration:underline;text-decoration-thickness:1px}
 #container del{color:#b91c1c;text-decoration:line-through}
 /* 打印 / 另存为 PDF：去掉深色底、工具栏、阴影与页间空隙，按纸张原样输出（页边距由文档
    自身的 sectPr 提供，故 @page 外边距为 0）。 */
 @page{margin:0}
 @media print{
  html,body{background:#fff !important}
  #toolbar{display:none !important}
  #container{padding:0 !important;overflow:visible !important}
  #container .docx-wrapper{background:#fff !important;padding:0 !important;margin:0 !important}
  #container section.docx,#container .docx-wrapper > .docx{box-shadow:none !important;margin:0 !important;
   page-break-after:always;break-after:page}
  #container .docx-wrapper > *:last-child{page-break-after:auto !important;break-after:auto !important}
  /* 屏幕上的等比缩放不能带进打印，否则整页被缩放/裁切 */
  #container ._dsh-docx-stage{transform:none !important;width:auto !important}
  #container *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
 }
</style>
</head><body>
<div style="position:sticky;top:0;z-index:20;display:flex;gap:6px;padding:8px 12px;align-items:center;flex-wrap:wrap;background:#1a2530" id="toolbar">
  ...
  <span style="display:flex;gap:4px;align-items:center;color:#9ca3af;font-size:12px">
    ...
    <button type="button" title="原始大小 100%" onclick="zoom100()" style="padding:4px 9px;border:1px solid #2c3a47;border-radius:7px;color:#e5e7eb;background:transparent;font-size:12px;cursor:pointer">100%</button>
    <button type="button" id="btnChanges" onclick="toggleChanges()" title="显示/隐藏 Word 修订标记（插入＝下划线、删除＝删除线）" style="padding:4px 9px;border:1px solid #2c3a47;border-radius:7px;color:#e5e7eb;background:transparent;font-size:12px;cursor:pointer">显示修订</button>
    <button type="button" onclick="window.print()" title="打印当前预览（浏览器里可另存为 PDF）" style="padding:4px 9px;border:1px solid #2c3a47;border-radius:7px;color:#e5e7eb;background:transparent;font-size:12px;cursor:pointer">打印</button>
  </span>
  <span title="预览为浏览器端重排：目录/页码等域不会重算；行距按 Word 语义校正（倍数×字体自然行高）；无分页标记的文档按纸张高度补算分页，表格与段落会按行拆到下一页（接近 Word，但不保证逐行一致）；「显示修订」按 Word 修订标记显示插入/删除（实验性）；需要与 Word 完全一致的版式请下载后用 Word 打开；「打印」可在浏览器里另存为 PDF。"域"不会重算；需要与 Word 完全一致的版式，请下载后用 Word 打开。" style="color:#8b98a5;font-size:12px;cursor:help">ⓘ 版式说明</span>
...
var zoom=1, fitMode=true, ZKEY='dsh-docx-view';
var buf=null, showChanges=false;
function dw(){
...
// 记住缩放（只记手动缩放；「适合宽度」是默认行为，不需要记）
function saveView(){ try{ localStorage.setItem(ZKEY, JSON.stringify({z:zoom, fit:fitMode})); }catch(e){} }
function zoomFit(){
...
function zoom100(){ zoom=1; fitMode=false; applyZoom(); saveView(); }
function applyFontFallback(root){
  var SERIF=/宋体|SimSun|仿宋|FangSong|楷体|KaiTi|宋体-简|Songti|serif|Times/i;
  var SERIF_FB=',"Songti SC","SimSun","Noto Serif CJK SC","Source Han Serif SC",serif';
  var SANS_FB=',"Microsoft YaHei","PingFang SC","Hiragino Sans GB","Noto Sans CJK SC","Source Han Sans SC","WenQuanYi Micro Hei",sans-serif';
  var all=root.querySelectorAll('*');
  for(var i=0;i<all.length;i++){
    var el=all[i]; if(!el.style) continue;
    var ff=el.style.fontFamily; if(!ff) continue;
    if(ff.indexOf('Songti SC')>=0||ff.indexOf('Microsoft YaHei')>=0||ff.indexOf('Noto ')>=0) continue;
    el.style.fontFamily=ff+(SERIF.test(ff)?SERIF_FB:SANS_FB);
  }
}
// ── 页脚页码域修复 ────────────────────────────────────────────────────────────
// docx-preview 会把含 fldChar / instrText 的整个 run 丢掉（renderRun: if(elem.fieldRun) return null）。
// python-docx 生成的页脚恰好把「域指令 + 缓存结果」放在同一个 run 里 ⇒ 页码整个消失，只剩「第  页」。
// 做法：从 footer XML 按文档顺序解析出「字面文本 / 域」序列（域带指令 PAGE/NUMPAGES/…），再按同样
// 顺序匹配渲染出的页脚 DOM：字面文本逐字对上；遇到域就在该处写入真实页码（若该处已有库渲染出来的
// 缓存数字，直接替换它）。匹配失败就把页脚还原，绝不改错。
var _footItems=null;
function _digitParts(rest){
  var t=(rest||'').trim();
  if(!/^[0-9]{1,4}$/.test(t)) return null;
  if(rest.replace(/[0-9]/g,'').trim()!=='') return null;      // 除数字外还有别的字符 → 不是缓存结果
  var i=rest.indexOf(t);
  return { pre:rest.slice(0,i), val:t, post:rest.slice(i+t.length) };
}
function _localName(el){
  var n=el.localName||el.nodeName||'';                   // 兼容不做命名空间解析的 XML DOM（localName 可能带前缀）
  var i=n.indexOf(':');
  return i>=0? n.slice(i+1) : n;
}
function _attrLocal(el, name){
  var a=el.attributes||[];
  for(var i=0;i<a.length;i++){ if(a[i].localName===name||a[i].name===name) return a[i].value; }
  return null;
}
function _inFldSimple(el){
  var p=el.parentNode;
  while(p && p.nodeType===1 && _localName(p)!=='p'){ if(_localName(p)==='fldSimple') return true; p=p.parentNode; }
  return false;
}
function parseFooterItems(xml){
  try{
    var xd=new DOMParser().parseFromString(xml,'application/xml');
    var all=xd.getElementsByTagName('*'), items=[], inFld=false;
    for(var i=0;i<all.length;i++){
      var el=all[i], ln=_localName(el);
      if(ln==='fldChar'){
        var ct=_attrLocal(el,'fldCharType');
        if(ct==='begin') inFld=true; else if(ct==='end') inFld=false;
        continue;
      }
      if(ln==='instrText'){ items.push({t:'field', instr:(el.textContent||'').trim()}); continue; }
      if(ln==='fldSimple'){ items.push({t:'field', instr:(_attrLocal(el,'instr')||'').trim()}); continue; }
      if(ln!=='t' && ln!=='delText') continue;
      if(inFld || _inFldSimple(el)) continue;        // 域内的缓存结果：库会丢掉，不能当字面量
      var txt=el.textContent||'';
      if(txt) items.push({t:'lit', text:txt});
    }
    return items;
  }catch(e){ return null; }
}
async function footerFields(){
  if(_footItems) return _footItems;
  var list=[];
  try{
    if(typeof JSZip==='undefined') return null;
    var zip=await JSZip.loadAsync(buf), names=[];
    zip.forEach(function(path){ var n=path.toLowerCase(); if(n.indexOf('word/footer')===0 && n.slice(-4)==='.xml') names.push(path); });
    names.sort();
    for(var i=0;i<names.length;i++){ list.push(parseFooterItems(await zip.file(names[i]).async('string'))); }
  }catch(e){ return null; }
  if(!list.length || !list[0]) return null;
  var has=false;
  for(var q=0;q<list[0].length;q++){ if(list[0][q].t==='field') has=true; }
  if(!has) return null;
  var sig=JSON.stringify(list[0]);
  for(var j=1;j<list.length;j++){ if(JSON.stringify(list[j])!==sig) return null; }   // 多个 footer 部件结构不一致 → 不冒险
  _footItems=list[0]; return _footItems;
}
function applyFootItems(foot, items, pageNo, total){
  var nodes=[], tw=document.createTreeWalker(foot, NodeFilter.SHOW_TEXT, null), n;
  while((n=tw.nextNode())) nodes.push(n);
  var ni=0, off=0;
  for(var k=0;k<items.length;k++){
    var it=items[k];
    if(it.t==='lit'){
      var want=it.text, w=0;
      while(w<want.length){
        if(ni>=nodes.length) return false;
        var cur=nodes[ni].nodeValue||'';
        if(off>=cur.length){ ni++; off=0; continue; }
        if(cur.charAt(off)!==want.charAt(w)) return false;
        off++; w++;
      }
      continue;
    }
    var v=/NUMPAGES|SECTIONPAGES/i.test(it.instr)? total : pageNo;
    if(ni>=nodes.length){ foot.appendChild(document.createTextNode(String(v))); continue; }
    var node=nodes[ni], rest=(node.nodeValue||'').slice(off), d=_digitParts(rest);
    if(off===0 && d){                                 // 该节点本身就是库渲染出的缓存数字
      node.nodeValue=d.pre+String(v)+d.post; ni++; off=0; continue;
    }
    if(off>0 && rest===''){                           // 游标正好在本节点末尾：下一个节点可能是缓存数字
      var nxt=nodes[ni+1], d2=nxt? _digitParts(nxt.nodeValue||'') : null;
      if(d2){ nxt.nodeValue=d2.pre+String(v)+d2.post; ni+=2; off=0; continue; }
      node.parentNode.insertBefore(document.createTextNode(String(v)), node.nextSibling); ni++; off=0; continue;
    }
    if(off>=node.nodeValue.length){                   // 空节点/末尾
      node.parentNode.insertBefore(document.createTextNode(String(v)), node.nextSibling); ni++; off=0; continue;
    }
    var tail=node.splitText(off);                     // 游标在节点中间 → 断开后插入
    tail.parentNode.insertBefore(document.createTextNode(String(v)), tail);
    nodes[ni]=tail; off=0;
  }
  return true;
}
async function fixPageFields(){
  try{
    var items=await footerFields(); if(!items) return;
    var pages=c.querySelectorAll('section.docx'); if(pages.length<1) return;
    for(var i=0;i<pages.length;i++){
      var foot=pages[i].querySelector('footer'); if(!foot) continue;
      var snap=foot.innerHTML;
      try{ if(!applyFootItems(foot, items, i+1, pages.length)) foot.innerHTML=snap; }
      catch(e){ foot.innerHTML=snap; }
    }
  }catch(e){ /* 修不了就保持原样，不影响预览 */ }
}
// ── 文档默认行距 / 单元格内边距（Word 有默认，库里没有）────────────────────────
// docx-preview 只在"段落自带 pPr 或引用了样式"时才写 line-height；python-docx 等生成的文档把行距
// 放在 docDefaults / Normal 样式里、段落本身不带 pPr ⇒ 预览里这些段落根本没有 line-height，退回
// 浏览器默认（中文看着很挤，表格最明显）。这里按文档默认行距（docDefaults，Normal 覆盖）与 Word 语义
// （倍数 × 字体自然行高）补上；表格单元格按 Word 表格样式的惯例用单倍（多数内置表格样式 pPr 就是
// line=240）。另补 Word 默认的单元格左右内边距（0.19cm），文档没写时文字不再贴着边框。
var _lineCtx=null;
function _mulOfXml(xml){
  try{
    var xd=new DOMParser().parseFromString(xml,'application/xml');
    var all=xd.getElementsByTagName('*');
    for(var i=0;i<all.length;i++){
      var el=all[i], p=el.parentNode;
      if(_localName(el)!=='spacing') continue;
      if(p && _localName(p)==='pPr'){
        var line=_attrLocal(el,'line'), rule=_attrLocal(el,'lineRule');
        if(line && (!rule || rule==='auto')){ var m=parseFloat(line)/240; if(m>0) return m; }
      }
    }
  }catch(e){}
  return null;
}
async function docLineSpacing(){
  if(_lineCtx) return _lineCtx;
  var ctx={body:1, table:1};
  try{
    if(typeof JSZip==='undefined'){ _lineCtx=ctx; return ctx; }
    var zip=await JSZip.loadAsync(buf);
    var stylesEl=zip.file('word/styles.xml'), docEl=zip.file('word/document.xml');
    var stylesXml=stylesEl? await stylesEl.async('string') : '';
    var docXml=docEl? await docEl.async('string') : '';
    if(stylesXml){
      var sd=new DOMParser().parseFromString(stylesXml,'application/xml');
      var sall=sd.getElementsByTagName('*');
      var m=null;
      for(var q=0;q<sall.length;q++){ if(_localName(sall[q])==='docDefaults'){ m=_mulOfXml(new XMLSerializer().serializeToString(sall[q])); break; } }
      var st=[];
      for(var q2=0;q2<sall.length;q2++){ if(_localName(sall[q2])==='style') st.push(sall[q2]); }
      for(var i=0;i<st.length;i++){
        if(_attrLocal(st[i],'styleId')==='Normal'){
          var mn=_mulOfXml(new XMLSerializer().serializeToString(st[i]));
          if(mn) m=mn;
          break;
        }
      }
      ctx.body=m||1;
      if(docXml){
        var tblStyleName=null;
        try{
          var xd= new DOMParser().parseFromString(docXml,'application/xml');
          var dall=xd.getElementsByTagName('*');
          for(var t=0;t<dall.length;t++){ if(_localName(dall[t])==='tblStyle'){ tblStyleName=_attrLocal(dall[t],'val'); break; } }
        }catch(e){}
        if(tblStyleName){
          for(var j=0;j<st.length;j++){
            if(_attrLocal(st[j],'styleId')===tblStyleName){
              var mt=_mulOfXml(new XMLSerializer().serializeToString(st[j]));
              if(mt) ctx.table=mt;
              break;
            }
          }
  ...
  _lineCtx=ctx; return ctx;
}
function applyDefaultLineHeights(ctx){
  try{
    var all=c.querySelectorAll('p');
    for(var i=0;i<all.length;i++){
      var el=all[i];
      if(el.style && el.style.lineHeight) continue;      // 库已按 pPr/样式给了 → 交给 fixLineHeights 缩放
      var inTable=false, pn=el.parentNode;
      while(pn && pn!==c){ if(pn.tagName==='TABLE'){ inTable=true; break; } pn=pn.parentNode; }
      var r=naturalLineRatio(el);
      el.style.lineHeight=((inTable?ctx.table:ctx.body)*r).toFixed(3);
      el.setAttribute('data-dsh-lh','1');                // 标记：避免被 fixLineHeights 二次放大
    }
  }catch(e){}
}
function applyDefaultCellPadding(){
  try{
    var cells=c.querySelectorAll('td, th');
    for(var i=0;i<cells.length;i++){
      var el=cells[i];
      if(!el.style || el.style.padding || el.style.paddingLeft) continue;
      el.style.paddingLeft='0.19cm'; el.style.paddingRight='0.19cm';
    }
  }catch(e){}
}
// ── 行距校正（Word 语义）────────────────────────────────────────────────────────
// docx-preview 把 w:spacing/@w:line 当成"字号倍数"：lineRule=auto 时直接 line-height = line/240。
// 但 Word 的语义是「倍数 × 字体自然行高」：单倍(240)≈1.3em(宋体)、1.5 倍(360)≈1.95em。
// 于是预览里的行距系统性偏紧（表格常用单倍 240 最明显，看着挤）。这里按元素字体量出"自然行高/字号"
// 的比例，把 docx-preview 写进去的无单位 line-height 乘上该比例，恢复 Word 观感。
// 只处理无单位数值（auto 规则）；atLeast 的 calc(...) 与固定值的 pt 不动。
var _lineRatio={};
function naturalLineRatio(el){
  var cs=getComputedStyle(el);
  var key=[cs.fontFamily,cs.fontSize,cs.fontWeight,cs.fontStyle].join('|');
  if(_lineRatio[key]!==undefined) return _lineRatio[key];
  var fs=parseFloat(cs.fontSize)||0, r=1.3;
  try{
    var probe=document.createElement('span');
    probe.textContent='汉字Hg';
  ...
  if(!(r>0.9)) r=1.3;
  if(r<1.0) r=1.0;
  if(r>1.7) r=1.7;                                  // 兜底：别因字体度量异常把行距拉飞
  _lineRatio[key]=r; return r;
}
function fixLineHeights(){
  try{
    var all=c.querySelectorAll('*');
    for(var i=0;i<all.length;i++){
      var el=all[i], st=el.style;
      if(!st || !st.lineHeight) continue;
      if(el.getAttribute && el.getAttribute("data-dsh-lh")) continue;   // 本文件补的默认行距，别重复放大
      if(!/^[0-9]*[.]?[0-9]+$/.test(st.lineHeight)) continue;   // 只改无单位倍数（auto）
      var m=parseFloat(st.lineHeight);
      if(!(m>0)) continue;
      var r=naturalLineRatio(el);
      st.lineHeight=(m*r).toFixed(3);
    }
  }catch(e){ /* 校正失败就维持库的默认，不影响预览 */ }
}
// ── 分页补算（PDF 式分页）──────────────────────────────────────────────────────
// docx-preview 只按 Word 存下的 lastRenderedPageBreak / 显式分页符切页；python-docx 等程序生成的
// 文档没有这些标记 ⇒ 整篇渲成"一长页"。这里按纸张内容区高（section 的 min-height 与 padding，
// docx-preview 已按文档 sectPr 算好）重新分页，并尽量贴近 Word：
//   · 放不下的表格按「行」拆到下一页；放不下的段落按「行」拆到下一页（续段不带编号）；
//   · 其余块（图、公式等）整块顺延到下一页。
// 只在"只渲染出一页、且内容高于一页"时启用；带分页标记的文档一律不动。
function _directChild(sec, tag){
  for(var i=0;i<sec.children.length;i++){ if(sec.children[i].tagName===tag) return sec.children[i]; }
  return null;
}
// 按行拆表：把放不下的后半张表克隆出来返回（原表被就地裁掉后半）；不可拆返回 null
function _splitTableRows(table, avail){
  var rows=table.querySelectorAll('tr');
  if(!rows || rows.length<2) return null;
  var top=table.getBoundingClientRect().top, fits=0;
  for(var i=0;i<rows.length;i++){
    var r=rows[i].getBoundingClientRect();
    if(r.bottom-top<=avail) fits=i+1; else break;
  }
  if(fits<1 || fits>=rows.length) return null;
  var rest=table.cloneNode(true), rr=rest.querySelectorAll('tr');
  var keep=Math.min(rows.length, rr.length);
  for(var j=fits-1;j>=0;j--){ if(rr[j] && rr[j].parentNode) rr[j].parentNode.removeChild(rr[j]); }   // 克隆只留"后半"的行
  for(var k=rows.length-1;k>=fits;k--){ rows[k].parentNode.removeChild(rows[k]); }                    // 原表只留"前半"的行
  return rest;
}
// 按行拆段：把放不下的后半段克隆出来返回（原段被就地裁掉后半，且续段不带列表编号）；不可拆返回 null
function _splitParagraphLines(p, avail){
  var probe=document.createRange();
  probe.selectNodeContents(p);
  var rects=probe.getClientRects();
  if(!rects || rects.length<2) return null;
  var top=p.getBoundingClientRect().top, fit=0;
  for(var i=0;i<rects.length;i++){ if(rects[i].bottom<=top+avail+1) fit++; else break; }
  if(fit<1 || fit>=rects.length) return null;
  var nodes=[], total=0, tw=document.createTreeWalker(p, NodeFilter.SHOW_TEXT, null), n;
  while((n=tw.nextNode())){ var len=(n.nodeValue||'').length; nodes.push({node:n, start:total, len:len}); total+=len; }
  if(total<2 || !nodes.length) return null;
  function posOf(k){
    for(var q=0;q<nodes.length;q++){ var t=nodes[q]; if(k<=t.start+t.len) return {node:t.node, off:k-t.start}; }
    var last=nodes[nodes.length-1]; return {node:last.node, off:last.len};
  }
  function linesOf(k){
    if(k<=0) return 0;
    if(k>total) k=total;
    var a=posOf(0), b=posOf(k), rr=document.createRange();
    try{ rr.setStart(a.node,a.off); rr.setEnd(b.node,b.off); }catch(e){ return 0; }
    var rs=rr.getClientRects();
    return rs? rs.length : 0;
  }
  var lo=1, hi=total, best=0;                 // 二分找"前 fit 行"对应的字符数
  while(lo<=hi){ var mid=(lo+hi)>>1; if(linesOf(mid)<=fit){ best=mid; lo=mid+1; } else { hi=mid-1; } }
  if(best<1 || best>=total) return null;
  var cut=posOf(best);
  var tail=cut.node.splitText(cut.off);
  var p2=p.cloneNode(false), cls=p2.getAttribute('class');
  if(cls){
    var parts=cls.split(' '), keepCls=[];
    for(var z=0;z<parts.length;z++){ if(parts[z] && parts[z].indexOf('docx-num')!==0) keepCls.push(parts[z]); }
    p2.setAttribute('class', keepCls.join(' '));
  }
  var sib=tail, move=[];
  while(sib){ move.push(sib); sib=sib.nextSibling; }
  for(var m=0;m<move.length;m++){ p2.appendChild(move[m]); }
  if(!(p2.textContent||'').trim()) return null;
  return p2;
}
async function paginatePages(){
  try{
    var wrapper=c.querySelector('.docx-wrapper'); if(!wrapper) return;
    var pages=wrapper.querySelectorAll('section.docx');
    if(pages.length!==1) return;                       // 库已分页 → 不动
    var first=pages[0], art=first.querySelector('article');
    if(!first||!art) return;
    var cs=getComputedStyle(first);
    var contentH=(parseFloat(cs.minHeight)||0)-(parseFloat(cs.paddingTop)||0)-(parseFloat(cs.paddingBottom)||0);
    if(contentH<=10) return;
    var flows=Array.prototype.slice.call(art.children), n=flows.length;
    if(n<2) return;
    // 量高度前先把屏幕缩放摘掉（transform 会影响 getBoundingClientRect）
    var savedTf=wrapper.style.transform, savedTo=wrapper.style.transformOrigin;
    wrapper.style.transform='none';
    var tops=[], hts=[], artTop=art.getBoundingClientRect().top;
    for(var i=0;i<n;i++){ var r=flows[i].getBoundingClientRect(); tops.push(r.top-artTop); hts.push(r.height); }
    var flowBottom=tops[n-1]+hts[n-1], adv=[];
    for(var j=0;j<n;j++){ adv.push((j+1<n?tops[j+1]:flowBottom)-tops[j]); }
    wrapper.style.transform=savedTf||''; wrapper.style.transformOrigin=savedTo||'';
    if(flowBottom<=contentH+2) return;                 // 一页装得下 → 不动
    var header=_directChild(first,'HEADER'), footer=_directChild(first,'FOOTER');
    var artClass=art.className, artStyle=art.getAttribute('style');
    var cur=art, used=0;
    function newPage(){
      var ns=first.cloneNode(false);
      if(!ns.className) ns.className='docx';
      wrapper.appendChild(ns);
      if(header) ns.appendChild(header.cloneNode(true));
      var na=document.createElement('article');
      if(artClass) na.className=artClass;
      if(artStyle) na.setAttribute('style',artStyle);
      ns.appendChild(na);
      if(footer) ns.appendChild(footer.cloneNode(true));
      cur=na; used=0;
    }
    function hOf(el){ return el.getBoundingClientRect().height||0; }
    // 拆分放置：前半留本页、后半到新页；余下部分仍超页就继续拆。成功返回 true
    function splitPlace(el, isTable){
      var rest=isTable? _splitTableRows(el, contentH-used) : _splitParagraphLines(el, contentH-used);
      if(!rest) return false;
      cur.appendChild(el);
      newPage();
      cur.appendChild(rest); used=hOf(rest);
      var guard=0;
      while(used>contentH+2 && guard++<300){
        var more=isTable? _splitTableRows(rest, contentH) : _splitParagraphLines(rest, contentH);
        if(!more) break;
        newPage();
        cur.appendChild(more); used=hOf(more); rest=more;
      }
      return true;
    }
    for(var q=0;q<n;q++){
      var el=flows[q], tag=(el.tagName||'').toLowerCase(), splittable=(tag==='table'||tag==='p');
      if(used>0 && used+adv[q]>contentH+2){
        if(splittable && splitPlace(el, tag==='table')) continue;
        newPage();
      }
      if(used<=0 && adv[q]>contentH+2 && splittable && splitPlace(el, tag==='table')) continue;
      cur.appendChild(el); used+=adv[q];
    }
  }catch(e){ /* 分页补算失败就维持"一长页"，不影响预览 */ }
}
function zoomBy(f){ zoom=Math.min(8, Math.max(0.05, zoom*f)); fitMode=false; applyZoom(); saveView(); }
// 「显示修订」：docx-preview 的 renderChanges（实验性）把 w:ins 渲染成 <ins>、w:del 渲染成 <del>。
function toggleChanges(){
  showChanges=!showChanges;
  var b=document.getElementById('btnChanges');
  if(b){
    b.textContent=showChanges?'隐藏修订':'显示修订';
    b.style.background=showChanges?'#2563eb':'transparent';
    b.style.borderColor=showChanges?'#2563eb':'#2c3a47';
  }
  render();
}
// 重新渲染（切换修订时复用已下载的文档，不重复请求）
async function render(){
  if(!buf) return;
  var sc=c.scrollTop||0;
  c.innerHTML='<div style="padding:40px;text-align:center;color:#999">渲染中…'
    +(buf.byteLength>2*1024*1024?'（文件较大，请稍候）':'')+'</div>';
  ...
  await docx.renderAsync(buf, c, null, {inWrapper:true, breakPages:true, ignoreLastRenderedPageBreak:false, experimental:true, className:'docx', useBase64URL:false,
    // 2026-09-14：把页眉/页脚/脚注/尾注也渲染出来，更接近 Word（docx-preview 默认不渲染这几项）
    renderHeaders:true, renderFooters:true, renderFootnotes:true, renderEndnotes:true,
    // 打印：外层容器样式按打印模式收起（版式由预览页的 @media print 接管）
    hideWrapperOnPrint:true,
    // 2026-09-14：修订标记（插入 <ins> / 删除 <del>），实验性；默认关，工具栏「显示修订」切换
    renderChanges:showChanges});
  // 中文字体回退：docx 里写的是 宋体/仿宋/黑体/楷体 等，客户端不一定有；给每处 font-family 追加回退链，
  // 避免字形与字宽不一致导致的行数/分页偏移（衬线族配衬线回退，其余配黑体/无衬线）。
  applyFontFallback(c);
  var _lc=await docLineSpacing();          // 读 docDefaults / Normal / 表格样式的行距
  applyDefaultLineHeights(_lc);            // 段落没有 line-height 时补上（Word 语义）
  applyDefaultCellPadding();               // 补 Word 默认的单元格左右内边距
  fixLineHeights();                        // 库按 pPr 写的倍数，乘上字体自然行高比例
  await paginatePages();   // 没有 lastRenderedPageBreak 的文档（如 md2docx 生成）按纸张高度补算分页
  await fixPageFields();   // 页脚页码域：按真实页序改写（docx-preview 只渲染缓存值）
  if(fitMode) zoomFit(); else applyZoom();   // 适合宽度（默认）或上次记住的缩放
  try{ c.scrollTop=sc; }catch(e){}
}
(async()=>{
  var st=null; try{ st=JSON.parse(localStorage.getItem(ZKEY)||'null'); }catch(e){}
  if(st && !st.fit && st.z){ zoom=st.z; fitMode=false; }   // 上次手动缩放，恢复；「适合宽度」不覆盖
  try{
    const r=await fetch(${JSON.stringify(inlineHref)}, {cache:'no-store',referrerPolicy:'no-referrer'});
...
// Ctrl+滚轮缩放（普通滚轮仍然是滚动页面）
window.addEventListener('wheel', function(e){ if(!e.ctrlKey) return; e.preventDefault(); zoomBy(e.deltaY<0?1.1:1/1.1); }, {passive:false});
</script>
</body></html>`);
    } catch (error) {
      sendError(res, error, onError);
...
window.addEventListener('resize',function(){ try{ if(fitMode) zoomFit(); else applyZoom(); }catch(e){} });
  ...
  const MODULES_DEFAULTS = { officePreview: true, uploadPreview: true, skillDocs: true, balance: true, mobile: true, workspace: true, turnRuler: false, sessionCost: false };
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const full = safeResolveInside(root, rel);
